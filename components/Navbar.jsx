        ...
        {open && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-6 py-4 space-y-4">
              <Link href="/" onClick={() => setOpen(false)} className="block text-slate-600 dark:text-zinc-300 hover:text-blue-600">Home</Link>
              <Link href="/complaintPage" onClick={() => setOpen(false)} className="block text-slate-600 dark:text-zinc-300 hover:text-blue-600">Complaints</Link>
              <Link href="/track" onClick={() => setOpen(false)} className="block text-slate-600 dark:text-zinc-300 hover:text-blue-600">Track Status</Link>
              <Link href="/about" onClick={() => setOpen(false)} className="block text-slate-600 dark:text-zinc-300 hover:text-blue-600">About</Link>
              <hr className="border-slate-200 dark:border-zinc-800" />
              {!isLoading && isAuthenticated ? (
                <button onClick={() => { handleSignOut(); setOpen(false); }} className="block w-full text-left text-slate-600 dark:text-zinc-300 hover:text-blue-600">Sign Out</button>
              ) : (
                <>
                  <Link href="/login" onClick={() => setOpen(false)} className="block text-slate-600 dark:text-zinc-300 hover:text-blue-600">Login</Link>
